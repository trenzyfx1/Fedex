    ...
    function movePointer() {
        ...
        deliveryRef.once("value").then(snap => {
            ...
            if (isPaused) {
                ...
                const now = Date.now();
                elapsed = elapsedBeforePause + Math.floor((now - (data.resumeAt || startTime)) / 1000);
            }
            ...
            updateCountdown(remaining, isPaused);
...
function toggleTracking() {
    const ref = firebase.database().ref("deliveries/" + code);
    ...
    ref.once("value").then(snap => {
        ...
        if (!isPaused) {
            // Pausing now
            const now = Date.now();
            const elapsedBeforePause = Math.floor((now - (data.resumeAt || data.startTime)) / 1000) + (data.elapsedBeforePause || 0);
            ref.update({
                trackingPaused: true,
                pausedAt: now,
                elapsedBeforePause: elapsedBeforePause
            }).then(() => {
                alert("⏸️ Paused successfully");
            });
        } else {
            // Resuming now
